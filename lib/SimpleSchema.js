...
class SimpleSchema {
  ...
  /**
   * @method SimpleSchema#optional
   * @param {[fields]} The list of fields to make optional in the subschema
   * @returns {SimpleSchema} The subschema
   */
  optional = getOptionalOrRequred('optional');

  /**
   * @method SimpleSchema#required
   * @param {[fields]} The list of fields to make required in the subschema
   * @returns {SimpleSchema} The subschema
   */
  required = getOptionalOrRequired('required');
...
function getPickOrOmit(type) {
  return function pickOrOmit(...args) {
    ...
    return new SimpleSchema(newSchema, this._constructorOptions);
  };
}

function getOptionalOrRequired(type) {
  return function optionalOrRequired(...args) {
    const newSchema = {};
    deepExtend(newSchema, this._schema);

    _.each(args, (value, key) => {
      newSchema[].optional = type === 'optional';
    });
